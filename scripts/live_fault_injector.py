...
def main(argv: list[str] | None = None) -> int:
    ...
    steps = sorted(
        [step for step in scenario.get("steps", []) if isinstance(step, dict)],
        key=lambda item: float(item.get("at_sec", 0.0)),
    )
    if args.dry_run:
        print(
            json.dumps(
    ...
    try:
        for index, step in enumerate(steps, start=1):
            scheduled_ns = started_mono_ns + int(float(step.get("at_sec", 0.0)) * 1_000_000_000)
            ...
            try:
                ...
                if details.get("status") == "error" and isinstance(step.get("fallback"), dict):
            ...
            _append_jsonl(
                args.output_jsonl,
                {
                    ...
                    "scenario": scenario.get("name"),
                    "note": step.get("note"),
                    "scheduled_at_sec": float(step.get("at_sec", 0.0)),
