...
* `B1`: transcript + `/compress` + hierarchical context
* `B2`: `Control View` without validity governor
* `B3`: `Control View` + TTL-only governor
* `B4`: full system = `Control View` + validity governor + action-state semantics + lease/commit guard + obligations

### E1. Sufficiency / relative minimality

**목적**
`Control View`가 family-level decision interface를 보존하는지, 그리고 어떤 slots가 실제로 indispensable한지 확인

**Steps**

1. PX4/MAVROS-specific family contracts를 hand-authoring
2. SITL에서 decision traces 수집
3. 각 decision point에 대해 oracle decision interface 생성
4. full `Control View`로 `verdict`, `canonical_args`, `commit_guard`, `blockers` 계산
...
### E2. Freshness / authority governance

**목적**
stale-state와 source disagreement를 validity semantics가 얼마나 잘 다루는지 검증

**Steps**

1. 다음 fault injection scenario 구성
   * pose message delay
...
2. `B2`, `B3`, `B4`를 동일 trace에 replay
3. 각 decision point에서 verdict, action-state progression, commit outcome 기록
4. refresh, safe_hold, refuse, abort 분포를 비교
...
* `source_disagreement_handling_rate`
* `weak_ack_without_confirm_rate`

### E3. Efficiency vs transcript memory

**목적**
chat history 기반 memory보다 typed `Control View`가 token/latency 측면에서 유리한지 검증

**Steps**

1. 동일 mission script를 `B0`~`B4`에 반복 실행
2. session logs, tool calls, token usage, latency를 수집
3. raw tool outputs와 prompt context 크기 비교
4. mission completion과 operator intervention 횟수 기록

**Metrics**

* `prompt_tokens_per_turn`
* `decision_latency_ms`
* `raw_tool_calls_per_mission`
* `mission_success_rate`
* `operator_override_count`

### E4. Obligation-mediated composition

**목적**
pending transitions를 explicit하게 들고 가는 것이 multi-step mission reliability에 도움이 되는지 검증
...
2. transition-specific fault 삽입
   * mode requested but not stabilized
   * takeoff acked but altitude not reached
   * OFFBOARD entered but no progress
   * land requested but touchdown not confirmed
3. obligation engine이 premature next-step act를 차단하는지 확인
4. blocker explanation과 recovery path를 기록
...
1. `Control View` full model은 trace class 내 family-level decision interface를 높은 정확도로 보존할 가능성이 크다.
...
3. TTL-only governor는 stale-state fault를 일부 줄이지만, reconnect, revision-based invalidation, weak-ack handling에서는 full governor보다 열세일 가능성이 크다.
4. full system은 transcript replay보다 prompt token과 raw tool chatter를 줄이고, stale-state unsafe act, weak-ack misuse, premature transition을 함께 낮출 가능성이 크다.
5. 효과의 핵심은 “더 많은 memory”가 아니라 “더 적은데 맞는 memory”라는 점으로 나타날 가능성이 크다.
...
* TTL-only vs full-governor replay
* stale pose replay
