...
* `guard`: `vehicle.connected`, `vehicle.armed`, `pose.local`, `estimator.health`, `failsafe.state`, `geofence.status`, `tf.local_body`, `offboard.stream.ok`, `mission.spec.rev`, `tool_registry.rev`
...
* `offboard.stream.ok`는 `publish_rate_hz`, `last_publish_age_ms`, `warmup_elapsed_ms`로만 계산한다.
  canonical payload shape는 `{"value": bool, ...}`이며 legacy `{"ok": bool, ...}`는 compatibility input으로만 허용한다.
...
* guard: `vehicle.connected`, `vehicle.mode`, `vehicle.armed`, `pose.local`, `estimator.health`, `failsafe.state`, `mission.spec.rev`, `tool_registry.rev`
* support: `battery.margin`
...
* guard: `vehicle.connected`, `vehicle.armed`, `pose.local`, `estimator.health`, `geofence.status`, `tf.local_body`, `failsafe.state`, `offboard.stream.ok`, `mission.spec.rev`, `tool_registry.rev`
* support: `battery.margin`
* confirm: `vehicle.mode`, `nav.progress`
...
* guard: `vehicle.connected`, `vehicle.mode`, `vehicle.armed`, `mission.spec.rev`, `tool_registry.rev`
* confirm: `vehicle.mode`, `nav.progress`
...
* guard: `vehicle.connected`, `vehicle.armed`, `home.ready`, `mission.spec.rev`, `tool_registry.rev`
...
* guard: `vehicle.connected`, `vehicle.mode`, `vehicle.armed`, `pose.local`, `estimator.health`, `mission.spec.rev`, `tool_registry.rev`
...
| --- | --- | --- | --- |
| `ARM_PENDING` | `ARM` enters `ACKED_STRONG` | `/mavros/state.armed == true` for 0.3s | not confirmed within 3.0s |
...
| `NAV_PENDING` | `GOTO` enters `ACKED_WEAK` | `vehicle.mode == "OFFBOARD"` and `nav.progress.phase == "ARRIVED"` for 0.5s | OFFBOARD lost before arrival, no progress within 3.0s, timeout `max(10.0, 2.0 * planned_distance_m + 5.0)` |
| `HOLD_PENDING` | `HOLD` enters `ACKED_WEAK` | mode == `AUTO.LOITER` and speed <= 0.3m/s for 1.0s | not confirmed within 2.0s |
| `RTL_PENDING` | `RTL` enters `ACKED_WEAK` | mode == `AUTO.RTL` | not confirmed within 5.0s |
| `LAND_PENDING` | `LAND` enters `ACKED_WEAK` | landed state `ON_GROUND` for 1.0s and disarm within 5.0s | `timeout_from_canonical_arg: land_timeout_sec`, minimum 30.0s |
...
arg_policy:
  ...
  canonicalization:
    GOTO:
      ...
      - fill_planned_distance_from_pose.local
      - fill_nav_timeout_sec: max(10.0, 2.0 * planned_distance_m + 5.0)
    TAKEOFF:
    ...
    LAND:
      - no_external_args
      - fill_land_timeout_sec: max(30.0, current_z / 1.2 + 12.0)
